...
import com.google.cloud.bigquery.{BigQuery, BigQueryError, BigQueryException, BigQueryOptions, DatasetId, ExternalTableDefinition, Field, FieldList, FormatOptions, Job, JobId, JobInfo, JobStatus, LoadJobConfiguration, QueryJobConfiguration, Schema, StandardSQLTypeName, TableId, TableInfo}
...
object BQ extends Logging {
  ...
  def getExecutionErrors(status: JobStatus): Seq[BQError] = {
    ...
    if (e == null) Seq.empty
    else e.asScala.flatMap(toBQError).toArray.toSeq
  }
